...
class FileChecker(object):
    ...
    def __init__(self, path, libcCheck=True, noVerbose = 0):
        self.path = path
    ...
    def checkBreakLine(self):
        exp = re.compile('^[ \t]*$')
        exp2 = re.compile('^[\t]*(struct |unsigned |long |static )*[a-zA-Z0-9_]+ [a-zA-Z0-9_\*\[\]]+( = [a-zA-Z0-9_\(\)\+\-\/%* \"\',{}\?\:\>\[\]]+)*;$')
        #exp2 = re.compile('^[\t]*(struct |unsigned |long |register )*[a-zA-Z0-9_]+ [a-zA-Z0-9_\*\[\]]+( = .+)*;$')
        ...
        if re.search(exp, self.line):
            ...
            if re.search(exp2, self.lastLine) and not re.search(exp2, self.line) and not self.line.endswith("\\"):
                self.printNormError("ligne vide manquante après la déclaration des variables")
    ...
    def checkSpaces(self):
        ...
        exp = re.compile('[ \t](if|else|return|while|for|switch|else if)[^ \t]')
        exp2 = re.compile('[ \t](if|else|return|while|for|switch|else if)[A-Za-z0-9_;]')
    ...
    def checkSpacesOperator(self):
        ...
        exp = re.compile("[^ \t\+\-=*%/!\'\(\[\{]+[\+\-=*%/]+[^ \t\+\-=*%/!\|[\]);><\']+|(^[\+-=/%]+$)")
        exp2 = re.compile("\"([^ \t\+\-=*%/!\'\(\[]+[\+\-=*%/]+[^ \t\+\-=*%/!\|[\]);><\']+|(^[\+-=/%]+$))")
        if self.isFound(re.search(exp, self.line)) and not self.isFound(re.search(exp2, self.line)) and not self.path.endswith('.h')\
    ...
    def checkProto(self):
        ...
        if res:
            ...
            if re.findall("[A-Z]+", name) and self.line.find('#define') == -1:
            ...
            if self.line[res.end(0) - 1] == ';' and not self.line.startswith("static"):
                ...
                if self.path.endswith(".h") and not self.line.endswith(';') and self.line.find('#define') == -1:
...
if len(sys.argv) > 1:
    ...
    for arg in sys.argv[1:]:
        args = arg.split("=")
        if args[0] == "--compile" and len(args) > 1:
            testCompile = 1
            compileRes = args[1]
...
errCount = 0
for path in files:
    f = FileChecker(path, False, noVerbose)
    errCount += f.check()
